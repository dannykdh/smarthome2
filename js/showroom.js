...
var Showroom = (function($, U) {
	...
	var service = {
		...
		config: {
			...
			PHONE_VIDEO_HEIGHT: 518,
			PHONE_VIDEO_WIDTH: 292,
			SHOW_DELAY: 1000,
	...
	Card.fillPropertiesForMainCard = function(card) {
		...
		card._resizePhoneHolder = function() {
			var state = service.state, config = service.config;
			...
			this.video.setSize({
				width: Math.round(Math.min(config.PHONE_VIDEO_WIDTH, this.$phoneHolder.width())),
				height: 'auto'
			});
		};
